...
class LSTM_Classifier(nn.Module):  # 创建一个种类，名字叫做LSTM_Classifier
...
class biGRU_ATTEN_Classifier(nn.Module):
    ...
    def forward(self, landmarks, lengths):
        ...
        rescaled_hidden, attention_weight = self.attention(query=ht, key=gru_output, value=gru_output)
...
class Self_Attention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        # print("===============")
        # print(int(batch_size[0]))
        key=key.reshape(int(length),int(batch_size[0].item()),-1)  # 修改
        value=key
        # import pdb
        # pdb.set_trace()
