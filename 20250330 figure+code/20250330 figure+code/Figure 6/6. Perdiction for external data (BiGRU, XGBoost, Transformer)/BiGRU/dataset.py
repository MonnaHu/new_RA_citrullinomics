from __future__ import absolute_import # from __future__ import *这样的语句。这样的做法的作用就是将新版本的特性引进当前版本中，也就是说我们可以在当前版本使用新版本的一些特性
...
class LandmarkList(data.Dataset):
    def __init__(self, root, fileList,transform=default_transform, list_reader=default_list_reader, loader=default_loader):
        ...
        self.normalize = np.load(r"H:/RA sera reactivity/sequential_model_pytorch-master/data/normalize.npy")
        ...
        data_dict = {"瓜氨酸": "X", "甘氨酸": "G", "丙氨酸": "A", "缬氨酸": "V", "亮氨酸": "L", "异亮氨酸": "I", "苯丙氨酸": "F", "色氨酸": "W",
                     ...
                     "组氨酸": "H", "精氨酸": "R"}
        df = pd.read_excel(r"H:/RA sera reactivity/sequential_model_pytorch-master/data/21个氨基酸性质(1).xlsx")
        data = df.values
        feature_dict = {}
        for value in data:
            name = value[1]
...
class LandmarkListSlipe(data.Dataset):
    def __init__(self, root, fileList,transform=default_transform, list_reader=slipe_list_reader, loader=default_loader):
        ...
        self.normalize = np.load("H:/RA sera reactivity/sequential_model_pytorch-master/data/normalize.npy")

        data_dict = {"瓜氨酸": "X", "甘氨酸": "G", "丙氨酸": "A", "缬氨酸": "V", "亮氨酸": "L", "异亮氨酸": "I", "苯丙氨酸": "F", "色氨酸": "W",
                     "酪氨酸": "Y", "天冬氨酸": "D", "谷氨酸": "E", "赖氨酸": "K", "天冬酰胺": "N",
                     "谷氨酰胺": "Q", "甲硫氨酸": "M", "丝氨酸": "S", "苏氨酸": "T", "半胱氨酸": "C", "脯氨酸": "P",
                     "组氨酸": "H", "精氨酸": "R"}
        df = pd.read_excel("H:/RA sera reactivity/sequential_model_pytorch-master/data/21个氨基酸性质(1).xlsx")
        data = df.values
        feature_dict = {}
        for value in data:
            name = value[1]
