...
for test_val_num in range(10):
    ...
    for epoch in tqdm(range(MAX_EPOCH), desc='Epochs', unit='epoch'):
        ...
        if test_acc > best_test:
            best_test = test_acc
            if SAVE_BEST_MODEL:
                torch.save(model.state_dict(), './' + rnn + '_LAYER' + str(N_LAYERS_RNN) + '_HIDDEN' + str(HIDDEN_DIM) + '_' + MODEL_NAME + '_epoch' + str(epoch) + '.pt')
                print('best epoch {}, train_acc {}, test_acc {}'.format(epoch, train_acc, test_acc))

df_result.to_csv('./All' + rnn + '_LAYER' + str(N_LAYERS_RNN) + '_HIDDEN' + str(HIDDEN_DIM) + '_' + MODEL_NAME + '.csv', index=False)
